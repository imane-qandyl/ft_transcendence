...
async function wsChatRoutes(fastify, options) {
  ...
  const addClient = (userId, socket) => {
    ...
    fastify.log.info(`[WS-Chat] Client added: userId=${userId}, totalClients=${clients.size}`);
  };
  ...
  const removeClient = (userId, socket) => {
    ...
    fastify.log.info(`[WS-Chat] Client removed: userId=${userId}, totalClients=${clients.size}`);
  };
  ...
  const sendToUser = (userId, obj) => {
    const set = clients.get(userId);
    if (!set) {
      fastify.log.warn(`[WS-Chat] User ${userId} not connected - cannot send message`);
      return;
    }
    fastify.log.info(`[WS-Chat] Broadcasting to user ${userId} (${set.size} connections)`, obj.type);
  ...
  fastify.get("/ws/chat", { websocket: true }, (connection, req) => {
    ...
    fastify.log.info("[WS-Chat] New connection (not authenticated yet)");
    ...
    connection.socket.on("message", async (raw) => {
      ...
      try {
        ...
        sendJSON(connection.socket, { type: "ERROR", payload: { message: "Invalid JSON" } });
        return;
      }
      ...
      if (!authedUserId) {
        if (msg.type !== "AUTH") {
          ...
          sendJSON(connection.socket, { type: "ERROR", payload: { message: "Send AUTH first" } });
          return;
        }
        ...
        if (!token || typeof token !== "string") {
          ...
          sendJSON(connection.socket, { type: "ERROR", payload: { message: "token required" } });
          return;
        }
        ...
        try {
          ...
          addClient(authedUserId, connection.socket);

          sendJSON(connection.socket, { type: "AUTH_OK", payload: { userId: authedUserId } });
          ...
          sendJSON(connection.socket, { type: "ERROR", payload: { message: "Invalid token: " + e.message } });
          connection.socket.close();
        }
      ...
      if (msg.type !== "CHAT_SEND") {
        ...
        sendJSON(connection.socket, { type: "ERROR", payload: { message: `Unknown type: ${msg.type}` } });
        return;
      }
      ...
      if (!chatId || typeof content !== "string" || !content.trim()) {
        fastify.log.error("[WS-Chat] CHAT_SEND validation failed:", { chatId, content: !!content });
        sendJSON(connection.socket, { type: "ERROR", payload: { message: "chatId + content required" } });
        return;
      }
      ...
      try {
        ...
        fastify.log.info(`[WS-Chat] ✓ Message created: id=${message.id}, sending to sender(${authedUserId}) and receiver(${receiverUserId})`);
        ...
        sendJSON(connection.socket, { type: "ERROR", payload: { message: e.message || "Failed to send message" } });
      }
    });
    ...
    connection.socket.on("close", () => {
      ...
      if (authedUserId) {
        ...
        removeClient(authedUserId, connection.socket);
      } else {
